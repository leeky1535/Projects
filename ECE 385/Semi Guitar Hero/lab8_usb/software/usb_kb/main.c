...
//	UsbPrintMem();
	IOWR(CY7C67200_BASE,HPI_ADDR,0x056E);
	code = IORD(CY7C67200_BASE,HPI_DATA);
	printf("\ncode = %x\n", code);
	code = code & 0x0ff;
//	code = code & 0x03;   // following tutorial and AND with 0x03
	if (code == 0x01)
	{
...
//	UsbPrintMem();
	IOWR(CY7C67200_BASE,HPI_ADDR,0x0579);
	data_size = 0x08;
//	data_size = (IORD(CY7C67200_BASE,HPI_DATA)>>8)&0x0ff;
//	data_size = (IORD(CY7C67200_BASE,HPI_DATA)>>8)&0x08;	//changed to 08 because data size is 0x08
...
//		IOWR(KEYCODE_BASE, 0, keycode & 0x0f);	//changed ff to 0f because reading lower byte of 16 bit value
